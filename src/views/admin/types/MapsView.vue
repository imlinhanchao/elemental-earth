      ...
      <div class="card-body p-4">
        <div class="flex items-center justify-between mb-3">
          ...
          <div class="flex gap-2 text-xs items-center">
            ...
            <span class="font-mono min-w-[3em] text-center"
            ...
            <button class="btn btn-xs btn-ghost" @click="resetPositions">
              重置布局
            </button>
            <button
        ...
        <div
          class="relative border border-base-300 rounded-lg overflow-hidden"
          ref="canvasRef"
          style="
            height: 420px;
        ...
        >
          <!-- 缩放容器 -->
          <div
            class="absolute inset-0 origin-top-left transition-transform duration-100"
          ...
          >
            ...
            <div
              ...
              :class="{
                'ring-2 ring-primary rounded-lg': draggingKey === r.key,
              }"
              :style="{
                left: (r.position?.x ?? 0) * 1.5 + 60 + 'px',
                top: (r.position?.y ?? 0) * 1.5 + 20 + 'px',
                zIndex: draggingKey === r.key ? 10 : 1,
              }"
              @mousedown.stop="startDrag($event, r)"
              @touchstart.stop.prevent="startDrag($event, r)"
            >
              <div
                class="w-14 h-14 rounded-xl bg-base-300 border border-base-300 flex items-center justify-center text-lg shadow-md hover:shadow-lg transition-shadow"
              ...
              <span
                class="text-[10px] font-medium px-1 truncate max-w-[80px] text-center leading-tight"
                >{{ r.name }}</span
              >
              <span class="text-[9px] opacity-40 font-mono"
                >{{ r.position?.x ?? 0 }}, {{ r.position?.y ?? 0 }}</span
              >
            </div>
          </div>
          <!-- /缩放容器 -->
          ...
          <div
            v-if="selectedNode"
            class="absolute bottom-2 right-2 bg-base-100 border border-base-300 rounded-lg p-3 shadow-lg z-20 flex gap-3 items-center text-xs"
          >
          ...
          <div class="space-y-4">
            ...
            <label class="form-control w-full">
              ...
              <input
                class="input input-bordered input-sm w-full"
                v-model="form.icon"
                placeholder="如 hugeicons:mountain"
              />
            </label>
            <label class="form-control w-full">
...
function startDrag(e: MouseEvent | TouchEvent, r: any) {
  ...
  const onMove = (ev: MouseEvent | TouchEvent) => {
    ...
    r.position.x = Math.max(
      0,
      Math.round(
        (p.clientX - rect.left - dragOffset.value.x) / (1.5 * zoom.value),
      ),
    );
    r.position.y = Math.max(
      0,
      Math.round(
        (p.clientY - rect.top - dragOffset.value.y) / (1.5 * zoom.value),
      ),
    );
    hasChanges.value = true;
  };
...
// ─── 表单编辑 ──────────────────────────────────────────────
function resetForm() {
  form.value = { key: "", name: "", icon: "", description: "", px: 0, py: 0 };
  editing.value = null;
}
function openNew() {
  resetForm();
...
async function save() {
  ...
  if (form.value.px || form.value.py)
    body.position = { x: form.value.px, y: form.value.py };
  try {
    const res = editing.value
      ? await admin.apiFetch(`/api/maps/${editing.value.key}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        })
      : await admin.apiFetch("/api/maps", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        });
...
async function remove(r: any) {
  if (!confirm(`确定删除地图「${r.name || r.key}」？`)) return;
  await admin.apiFetch(`/api/maps/${r.key}`, { method: "DELETE" });
  loadRecords();
}
</script>
