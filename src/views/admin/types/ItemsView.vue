  ...
  <div class="flex-1 overflow-y-auto p-4">
    <div class="flex items-center gap-3 mb-6">
      ...
      <h2 class="text-xl font-bold inline-flex items-center gap-1">
        <Icon icon="tabler:flask" class="inline-block align-middle mr-1" />物品管理
      </h2>
      <span class="badge badge-ghost badge-sm">{{ records.length }} 条</span>
    </div>
    <div class="flex gap-2 mb-4">
      <button class="btn btn-primary btn-sm" @click="openNew">＋ 新增物品</button>
      <button class="btn btn-ghost btn-sm" @click="loadRecords">⟳ 刷新</button>
    </div>
    <div class="overflow-x-auto rounded-box border border-base-300">
      <table class="table table-zebra table-xs">
        <thead><tr><th>标识符</th><th>名称</th><th>分类</th><th>类型标签</th><th>元素</th><th class="w-28">操作</th></tr></thead>
        <tbody>
          <tr v-for="r in records" :key="r.key">
            <td class="font-mono text-xs">{{ r.key }}</td>
            <td>{{ r.name }}</td><td>{{ r.category }}</td>
            <td><span v-for="t in (r.type||[])" :key="t" class="badge badge-ghost badge-xs mr-0.5">{{ t }}</span></td>
            <td>{{ r.elemental ?? '—' }}</td>
    ...
    <Teleport to="body">
      <dialog ref="modalRef" class="modal">
        <div class="modal-box max-w-xl">
          <h3 class="text-lg font-bold mb-6">{{ editing ? '编辑物品' : '新增物品' }}</h3>
          <div class="space-y-4 max-h-[65vh] overflow-y-auto pr-1">
            <div class="grid grid-cols-2 gap-4">
              <label class="form-control w-full">
                ...
                <input class="input input-bordered input-sm w-full" v-model="form.key" placeholder="英文标识" />
              </label>
              <label class="form-control w-full">
            ...
            <label class="form-control w-full">
              ...
              <div class="flex flex-wrap gap-1">
                <span v-for="opt in typeOptions" :key="opt" class="badge badge-ghost badge-sm cursor-pointer hover:badge-primary"
                  :class="{ 'badge-primary': form.type.includes(opt) }" @click="toggleType(opt)">{{ opt }}</span>
              </div>
            </label>
            <div class="grid grid-cols-2 gap-4">
              <label class="form-control w-full">
                <span class="label-text mb-1">元素序数</span>
                <input type="number" class="input input-bordered input-sm w-full" v-model.number="form.elemental" placeholder="可选" />
              </label>
              <label class="form-control w-full">
                <span class="label-text mb-1">耐久度</span>
                <input type="number" step="0.01" class="input input-bordered input-sm w-full" v-model.number="form.durable" placeholder="0-1" />
              </label>
            </div>
            <label class="form-control w-full">
...
async function remove(r: any) { if (!confirm(`确定删除物品「${r.name||r.key}」？`)) return; await admin.apiFetch(`/api/items/${r.key}`, { method:'DELETE' }); loadRecords() }
</script>
