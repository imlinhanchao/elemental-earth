        ...
        <tbody>
          <tr v-for="r in records" :key="r.key">
            ...
            <td>
              <div class="flex flex-wrap gap-1">
                <span
                  v-for="t in r.required_techs || []"
                  :key="t"
                  class="badge badge-ghost badge-sm"
                >
                  {{ techOptions.find(o => o.key === t)?.name || t }}
                </span>
              </div>
            </td>
            <td>
          ...
          <div class="space-y-4 max-h-[65vh] overflow-y-auto pr-1">
            ...
            >
              ...
              <div class="flex-1 min-w-[100px]">
                <div class="flex flex-wrap gap-0.5 mb-0.5">
                  <LabelTag
                    v-for="(k, ki) in row._reqItemKeys"
                    :key="ki"
                    :options="itemOptions"
                    :value="k"
                    closeable
                    @close="row._reqItemKeys.splice(ki, 1)"
                  ></LabelTag>
                  <span
                    v-if="!row._reqItemKeys.length"
              ...
              <div class="flex-1">
                <div class="flex flex-wrap gap-1 mb-1">
                  <div
                    v-for="(entry, ti) in row._mapEntries"
                    :key="ti"
                    class="flex items-center gap-0.5 badge badge-ghost badge-sm px-1"
                  >
                    <span class="text-[10px]">{{ getMapName(entry.key) }}</span>
                    <input
                      type="number"
                      class="input input-xs w-12 h-4 text-[10px] px-0.5 bg-transparent border-0 border-b border-base-content/20"
                      v-model.number="entry.probability"
                      placeholder="权"
                      title="覆盖概率，留空使用默认权重"
                    />
                    <button
                      class="text-xs leading-none hover:text-error"
                      @click="row._mapEntries.splice(ti, 1)"
                    >
                      ✕
                    </button>
                  </div>
            ...
            <div class="flex gap-4">
              ...
              <label class="form-control flex-1">
                ...
                <div class="flex flex-wrap gap-1 mb-1">
                  <LabelTag
                    v-for="(t, i) in form.maps"
                    :key="i"
                    :options="mapOptions"
                    :value="t"
                    closeable
                    @close="form.maps.splice(i, 1)"
                  />
                </div>
                <SearchableSelect
                  :options="mapOptions"
            ...
            <div class="grid grid-cols-2 gap-4">
              <label class="form-control w-full">
                ...
                <select
                  ...
                  v-model="form.formula_key"
                >
                  <option value="">不引用配方</option>
                  <option
                    v-for="o in formulaOptions"
                    :key="o.key"
                    :value="o.key"
                  >
                    {{ o.name }}
              ...
              <label class="form-control w-full">
                ...
                <select
                  class="select select-bordered select-sm w-full"
                  v-model="form.formula_op"
                >
                  <option value="">—</option>
                  <option v-for="o in labOptions" :key="o.key" :value="o.key">
                    {{ o.name }}
                  </option>
                </select>
              </label>
...
function getMapName(key: string): string {
  return mapNameMap.value[key] || key;
}
...
async function loadRecords() {
  const r = await admin.apiFetch("/api/actions");
...
function openEdit(r: any) {
  ...
  Object.assign(form, {
    ...
    time_required: r.time_required ?? 10,
    cooldown: r.cooldown || 0,
    techs: [...(r.required_techs || [])],
    maps: [...(r.map || [])],
    formula_key: r.formula?.key || "",
    formula_op: r.formula?.operation || "",
  });
  objItems.push(
  ...
  objRewards.push(
    ...(r.rewards || []).map((x: any) => {
      const reqKeys = Array.isArray(x.required_item)
        ? [...x.required_item]
        : x.required_item
          ? [x.required_item]
          : [];
      return {
        ...x,
        _mapEntries: (x.map || []).map((m: any) =>
          typeof m === "string"
            ? { key: m, probability: undefined }
            : { key: m.key, probability: m.probability },
        ),
        _mapInput: "",
...
function addMap() {
  if (mapInput.value && !form.maps.includes(mapInput.value))
    form.maps.push(mapInput.value);
  mapInput.value = "";
}
...
function addRewardMap(row: any) {
  const v = row._mapInput;
  if (v && !row._mapEntries.some((e: any) => e.key === v))
    row._mapEntries.push({ key: v, probability: undefined });
  row._mapInput = "";
}
function addReqItem(row: any) {
  const v = row._reqItemInput;
  if (v && !row._reqItemKeys.includes(v)) row._reqItemKeys.push(v);
  row._reqItemInput = "";
}
...
async function save() {
  ...
  if (objRewards.length)
    body.rewards = objRewards.map((r: any) => {
      ...
      o.quantity = parseQty(r.quantity);
      ...
      if (r._mapEntries?.length)
        o.map = r._mapEntries.map((e: any) => {
          if (e.probability !== undefined && e.probability !== "")
  ...
  if (form.formula_key && form.formula_op)
    body.formula = { key: form.formula_key, operation: form.formula_op };
  try {
    const res = editing.value
      ? await admin.apiFetch(`/api/actions/${editing.value.key}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        })
      : await admin.apiFetch("/api/actions", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        });
...
async function remove(r: any) {
  if (!confirm(`确定删除行动「${r.name || r.key}」？`)) return;
  await admin.apiFetch(`/api/actions/${r.key}`, { method: "DELETE" });
  loadRecords();
}
</script>
