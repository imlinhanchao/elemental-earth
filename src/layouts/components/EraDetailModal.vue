...
const currentEraData = computed(() =>
  Eras.find((e) => e.key === currentEraKey.value),
);
...
// 右侧卡片：当前查看的主角时代
const viewingEra = computed(() =>
  Eras.find((e) => e.order === viewingIndex.value),
);
// 左侧卡片：焦点时代的上一个时代
const prevEraPreview = computed(() =>
  Eras.find((e) => e.order === viewingIndex.value - 1),
);
...
<template>
  ...
  >
    <div
      class="modal-box max-w-4xl bg-base-100 p-0 overflow-hidden flex flex-col h-[700px]"
    >
      <!-- 头部控制 -->
      <header
        class="p-4 bg-base-200 border-b border-base-300 flex items-center justify-between shrink-0"
      >
        <div class="flex items-center gap-4">
          ...
          <div class="join bg-base-100 border border-base-300">
            ...
            <div
              class="px-3 flex items-center text-xs font-mono min-w-24 justify-center"
            >
              焦点纪元: {{ viewingIndex + 1 }}
            </div>
            <button
              class="btn btn-xs join-item"
              :disabled="viewingIndex >= (currentEraData?.order || 0)"
              @click="nextEra"
            >
      ...
      <div class="flex-1 overflow-y-auto p-6 bg-base-300/30">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          ...
          <div
            v-if="prevEraPreview"
            class="card bg-base-100 shadow-xl border border-base-300 opacity-80 scale-95 origin-right transition-all"
          >
            <div class="card-body p-6">
              <div class="flex items-start justify-between mb-4">
                <div class="flex items-center gap-3">
                  <div
                    class="w-12 h-12 rounded-xl bg-success/10 text-success flex items-center justify-center"
                  >
                    <Icon :icon="prevEraPreview.icon" class="text-3xl" />
                  </div>
                  <div>
                    <h2 class="card-title text-xl font-black opacity-60">
                      {{ prevEraPreview.name }}
                    </h2>
                    <div class="badge badge-success badge-xs">历史纪元</div>
                  </div>
                </div>
                <div class="text-right font-mono text-sm opacity-50">
                  #{{ prevEraPreview.order + 1 }}
                </div>
              </div>

              <p class="text-sm opacity-40 mb-6 min-h-[3em]">
                {{ prevEraPreview.description }}
              </p>
          ...
          <div
            v-else
            class="card bg-base-200 border border-dashed border-base-300 h-full flex items-center justify-center min-h-[400px] opacity-30"
          >
          ...
          <div
            v-if="viewingEra"
            class="card bg-base-100 shadow-2xl border-2 border-primary/20 relative z-20"
          >
            <div class="card-body p-6">
              <div class="flex items-start justify-between mb-4">
                <div class="flex items-center gap-3">
                  <div
                    ...
                    :class="
                      isEraCompleted(viewingEra.order)
                  ...
                  <div>
                    <h2 class="card-title text-xl font-black">
                      {{ viewingEra.name }}
                    </h2>
                    <div class="flex gap-1 mt-0.5">
                      <div
                        v-if="isEraCompleted(viewingEra.order)"
                        class="badge badge-success badge-xs"
                      >
                        已达成
                      </div>
                      <div v-else class="badge badge-primary badge-xs">
                        演进中
                      </div>
                    </div>
                  </div>
              ...
              <p class="text-sm opacity-70 mb-6 min-h-[3em]">
                {{ viewingEra.description }}
              </p>
              <template v-if="viewingEra.milestones.length > 0">
                <!-- 时代特质 -->
                <div class="flex flex-wrap gap-2 mb-6">
                  <div
                    v-if="viewingEra.order > 0"
                    class="badge badge-outline border-primary/30 text-primary text-[10px] py-2 gap-1 px-2 font-bold"
                  >
                ...
                <div class="space-y-2 mb-6">
                  <div
                    class="flex justify-between text-[10px] uppercase font-bold tracking-widest opacity-40"
                  >
                    <span>时代完成进度</span>
      ...
      <!-- 底部提示 -->
      <footer
        class="p-4 bg-base-200 border-t border-base-300 flex justify-center shrink-0"
      >
