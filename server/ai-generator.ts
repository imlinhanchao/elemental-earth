...
- **核心材料标记**：配方中的主要反应物（例如冶炼时的矿石、合成时的底物）必须设置 \`"isMain": true\`。这决定了玩家在接触到这些材料前不会掉落该配方碎片。- **耗时规则**：\`time_required\` 必须等于 **实验室操作耗时 * 所需最少操作次数 (action_min)**。请确保逻辑一致。- required_container 用于需要特定容器的反应（窑炉、烧杯、蒸馏瓶等）
